...
(defn- sq-dist-buf
  ...
  (let [start (long (* i 14))
        d0  (- (* (double (int (.get vectors (int start))))        scale-inv) (double (aget query  0)))
        d1  (- (* (double (int (.get vectors (int (+ start  1))))) scale-inv) (double (aget query  1)))
        d2  (- (* (double (int (.get vectors (int (+ start  2))))) scale-inv) (double (aget query  2)))
        d3  (- (* (double (int (.get vectors (int (+ start  3))))) scale-inv) (double (aget query  3)))
        d4  (- (* (double (int (.get vectors (int (+ start  4))))) scale-inv) (double (aget query  4)))
        d5  (- (* (double (int (.get vectors (int (+ start  5))))) scale-inv) (double (aget query  5)))
        d6  (- (* (double (int (.get vectors (int (+ start  6))))) scale-inv) (double (aget query  6)))
        d7  (- (* (double (int (.get vectors (int (+ start  7))))) scale-inv) (double (aget query  7)))
        d8  (- (* (double (int (.get vectors (int (+ start  8))))) scale-inv) (double (aget query  8)))
        d9  (- (* (double (int (.get vectors (int (+ start  9))))) scale-inv) (double (aget query  9)))
        d10 (- (* (double (int (.get vectors (int (+ start 10))))) scale-inv) (double (aget query 10)))
        d11 (- (* (double (int (.get vectors (int (+ start 11))))) scale-inv) (double (aget query 11)))
        d12 (- (* (double (int (.get vectors (int (+ start 12))))) scale-inv) (double (aget query 12)))
        d13 (- (* (double (int (.get vectors (int (+ start 13))))) scale-inv) (double (aget query 13)))
        s0  (+ (* d0  d0)  (* d1  d1))
        s1  (+ (* d2  d2)  (* d3  d3))
        s2  (+ (* d4  d4)  (* d5  d5))
        s3  (+ (* d6  d6)  (* d7  d7))
